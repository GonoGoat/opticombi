...
particleStruct Algo_PSO(mapStruct *mapParams, psoStruct *psoParams)
{
	...
	std::uniform_int_distribution<int> dist8(-7, 7);
		...
		for (int i = 0; i < psoParams->nbr_thread; i++)
		{
			...
			if (int(particles[i].distance_finish) == 0 && particles[i].become_finish == false)
			{
				...
				for (int j = psoParams->nbr_particule; j > 0; j--)
				{
					...
					particles[psoParams->nbr_thread - j].success = En_vie;
				}
			}

			// Vérification de jonction avec une particule démarrant à la base
			for (int j = 0;j<psoParams->nbr_base;j++) {
...
particleStruct Algo_PSO_bis(mapStruct *mapParams, psoStruct *psoParams, particleStruct* partParams)
{
	...
	for (int i = 0; i < psoParams->nbr_thread; i++)
	{
		...
		if (particles[i].success == Mort) {
			...
			if (particles[i].Output == "") {
				particles[i].Direction_tank = partParams->Direction_tank;
				particles[i].Direction_original_tank = partParams->Direction_original_tank;
			}
			else {
				particles[i].Direction_tank = particles[i].Output.back();
			}
	...
	while (nbr_iteration_t < psoParams->nbr_iteration_max)
	{
		for (int i = 0; i < psoParams->nbr_thread; i++)
		{
			...
			// Vérification de jonction avec une particule démarrant à la base
			if (int(particles[i].distance_finish) == 0) {
				std::cout << "Chemin trouve!" << std::endl;
				found_finish = true;
				num_particule = i;
				break;
...
int getBestParticleAlive(std::vector<particleStruct>* particles, int* nbr_base) {
	...
	int bestOutputSize = NULL;

	for (int i = 0; i < *nbr_base; i++) {
		if (((bestDistance == NULL || (*particles)[i].distance_finish < bestDistance) || ((*particles)[i].distance_finish == bestDistance && bestOutputSize > (*particles)[i].Output.size())) && (*particles)[i].success != Mort) {
			bestPart = i;
			bestDistance = (*particles)[i].score;
			bestOutputSize = (*particles)[i].Output.size();
		}
	}
	return bestPart;
}